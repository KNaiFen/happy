...
- 验收证据：App 全量源码 Vitest 110 个文件、1046 项断言通过；10 种语言的 857 个 key 形状一致；
  当前 `origin/main` 已包含该实现并发布 App `1.11.47`，对应成功发行编排为
  [31658286903](https://github.com/KNaiFen/happy/actions/runs/31658286903)。

## 背景与证据

实施前的 `MetadataSchema` 只接受字面量 `codexSyncVersion=4`，导致明确的旧 Codex
...
5. [x] App 源码测试、GitHub Actions 与 App `1.11.47` 发行完成后归档本计划并更新索引。
