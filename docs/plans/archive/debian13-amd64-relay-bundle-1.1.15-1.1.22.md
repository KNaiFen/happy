# Debian 13 amd64 Happy Relay 1.1.15-1.1.22 归档

> 归档于 2026-07-30。本文保留最初发行到 `1.1.22` 的完整构建与验收记录，
> 不再作为当前工作清单。活动计划见
> `docs/plans/debian13-amd64-relay-bundle.md`。
...
10. 首次发行 `1.1.15` 被 ShellCheck 门禁阻断；`1.1.16` 缺少 App 跨包契约
    schema；`1.1.17` 被 runtime Critical CVE 门禁阻断；`1.1.22` 在物理设备
    验收中暴露 production deploy 依赖漂移和 `BYTEA` 读取故障。修复版本目标为
    Server `1.1.23`。CLI 因配套离线/daemon 修复推进到 `1.4.6`；App
    `1.11.11`、Wire `0.1.3` 不因纯 Server 打包变化推进版本。
...
- `/health` 必须执行合成 `BYTEA` 解码探针；空业务表上的 `SELECT 1` 不足以
  声明数据库健康。
- 通过真实 HTTP 认证创建含非空 `dataEncryptionKey` 的 Machine 与 Session，
  再通过 App/CLI 使用的列表接口读回并逐字节核对；容器重启后重复读取。
- 最终 production deploy tree 和镜像内的 Prisma、PGlite 与 adapter 版本必须
  与根 lockfile 的已验证组合完全一致，禁止 semver 范围在 deploy 阶段漂移。
- 使用 PGlite `0.3.16` 写入的既有数据目录模拟 `1.1.22` 用户卷，升级到
  `1.1.23` 后确认账户、Machine、Session 和非空密钥无损保留。
...
- 2026-07-30：物理设备验收重新打开本计划。`1.1.22` 的
  ...
  `0.3.15` 漂移为 `0.3.16`，并把 Prisma `6.19.2` 与仅支持 Prisma 7 的
  adapter 组合。非空 `dataEncryptionKey` 读取触发 `P2023`，导致 Machine 与
  Session API 500；现有 `/health` 的 `SELECT 1` 和 marker 持久化测试未覆盖
  该业务路径。目标推进到 `1.1.23`，完成 production 依赖闭包、深度健康、
  非空加密字段、已有卷升级和真实设备链路验收后才能再次关闭。
