...
- 开始日期：2026-07-28
- 当前状态：R8 设备接入修复进行中；首次 Debian 物理设备验收发现生产
  PGlite 依赖漂移、加密二进制字段读取失败及 CLI 离线契约缺口，
  `HAPPY_CODEX_SYNC_V4_ENABLED` 保持关闭
...
### R8 设备接入与 PGlite 发行修复

2026-07-30 的首次物理设备验收确认，Debian relay `1.1.22` 的源码测试与浅层
`/health` 虽然通过，但最终 `pnpm deploy --prod --legacy` 生产树把
`@electric-sql/pglite` 从根锁文件中的 `0.3.15` 漂移为 `0.3.16`，并组合了
仅声明支持 Prisma 7 的 `pglite-prisma-adapter 0.7.2` 与 Prisma `6.19.2`。
非空 `BYTEA` 因而以数字键对象返回，Prisma 对 Session/Machine 的
`dataEncryptionKey` 抛出 `P2023`。认证和 Socket 可以成功，但
`GET /v1/machines`、`GET /v1/sessions` 与后续注册均失败，App 停留在添加设备
页面。CLI 的离线 session stub 同时缺少 `onFileEvent` 等公开契约，随后产生
二次崩溃。

- [ ] Server 精确锁定与 Prisma `6.19.2` 同代的 PGlite adapter 及已验证的
      PGlite runtime；根 lockfile、最终 production deploy tree 和镜像内版本
      必须一致，禁止 `^` 范围在发行阶段静默漂移。
- [ ] 用非空 Session、Machine、Artifact 及其他 Prisma `Bytes` 字段验证写入、
      同进程读取、关闭后重开读取和已有 `0.3.16` 数据目录无损读取；不得清库、
      重建账户、重生成 master secret 或改写已有密钥。
- [ ] `/health` 增加不依赖业务表是否为空的 `BYTEA` 解码探针，并继续返回固定、
      无敏感信息的 503；Docker healthcheck 和安装器必须以该深度结果为准。
- [ ] Debian 最终 tarball 生命周期使用真实 HTTP 认证、非空加密 Machine 与
      Session 创建、列表读取、容器重启和再次读取；仅 `SELECT 1`、目录存在或
      marker 文件不再构成数据库业务验收。
- [ ] 云端验证 `1.1.22`/PGlite `0.3.16` 已写卷升级到新镜像后原 ID、密钥字节、
      Machine/Session 数量和 v4 capability 均保持一致。
- [ ] CLI 离线 session 使用显式、编译期校验的完整公开契约；Server 暂时失败时
      `happy codex` 可保持离线重连，不因缺少 `onFileEvent` 或后续附件/RPC/
      metadata 方法崩溃。
- [ ] daemon state 加入规范化 server origin 身份并在 `daemon start` 时参与
      同实例判断；相同 CLI 版本但 relay 不同必须先协调停止旧 daemon，再启动
      新实例，父进程不得把任意陈旧 state 当作成功。
- [ ] access credential 与规范化 server origin 绑定。旧凭据先通过服务端验证；
      origin 不符或 401 时给出明确重新登录操作，不把一个 relay 的 bearer token
      静默发送给另一个 relay。
- [ ] Machine 注册 5xx 不得伪报“registered”。允许 daemon 进入持久重连态，
      但只有收到权威 Machine 响应后才能发布已注册状态和依赖它的 RPC 能力。
- [ ] App 扫码授权、Machine 注册、设备列表刷新、Codex Session 创建及 v4
      capability 协商纳入同一真实链路；验证 App 初始添加设备状态最终转为已连接，
      CLI/Server 重启后仍可恢复。
- [ ] R8 目标版本为 CLI `1.4.6`、Server `1.1.23`；App `1.11.11` 与 Wire
      `0.1.3` 仅在实际修改其可分发代码时再推进 patch。

## 发布顺序

1. 修复代码并保持 v4 Server flag 关闭。
2. 推进受影响包 patch 版本；本轮最低目标：
   CLI `1.4.6`、Server `1.1.23`；App 保持 `1.11.11`、Wire 保持
   `0.1.3`，除非 R8 实际修改对应可分发代码。
...
- 2026-07-30：Debian `1.1.22` 物理设备验收发现最终 production deploy tree
  将 PGlite 漂移到 `0.3.16`，非空 `BYTEA` 触发 Prisma `P2023`，使 Machine
  和 Session 列表同时返回 500；CLI 离线 stub 又因缺少 `onFileEvent` 崩溃。
  本地已使用镜像依赖组合精确复现，并确认锁定的 `0.3.15` 可以无损读取
  `0.3.16` 写入的数据目录。新增 R8 发行依赖闭包、深度健康、已有卷升级、
  完整离线契约、daemon/credential origin 隔离和真实设备链路门禁。
