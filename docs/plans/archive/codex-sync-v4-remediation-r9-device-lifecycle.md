# Codex Sync v4 R9 现场故障修复计划（已归档）

## 状态

- 当前状态：已完成并归档。R9 实现、审查、云端验证和发行交付已闭环；后续实体设备抽检属于 release 后外部验收，不再构成本实施计划的未完成项。
...
- [x] relay 离线 bundle 验收脚本使用真实 terminal auth
      request/approval 获得终端 token 后再注册 Machine，不得复用
      App/account token；创建终端 Session 时显式携带已绑定的 `machineId`。
- [x] 跟踪 GitHub Actions，修复至 required CI、CLI、Server/relay 和 Android
...
- 最终分支 CI `30532495242`、main CI `30532562048` 全绿；覆盖 stable-v2
  协议漂移、真实十分钟 turn、Codex 传输场景、数据库升级、Server runtime、
  App Web、CLI 和 macOS Tauri/Rust。
- CLI workflow `30529266942` 成功；`happy-1.4.7.tgz` 已下载并验证包内版本、
  macOS ARM64 `rg`/`difftastic`，SHA-256 为
  `bb9d4422b451156c36e5da705bd0209d0926e55966ae250ce3ddb4b1c2782a93`。
- Android workflow `30529266997` 成功；Artifact
  `happy-app-1.11.12-android-arm64-v8a-no-ota`，ID `8754738280`。
- Relay workflow `30532561673` 成功；离线 bundle 已完成安装、真实 terminal
  auth、Machine/Session、重启持久化、安全限制和 v4 开关验收。下载文件
  `happy-relay-server-1.1.27-debian13-amd64.tar.gz` 的 SHA-256 为
  `3e70412696e4b59383a72f519e0114343d7e8f24d6b28eead9b1d1976a12ca64`。
