...
- 负责人：当前任务主代理。
- 当前授权：编写 PLAN、同步文档索引、必要本地校验和文档提交。
- 下一步：批准首轮实施范围后，从最新 `origin/main` 核对差异并执行下述 A1-A5，再按 B1 的证据条件决定后续范围。
- 本轮不修改工作流、测试或平台设置，不触发、重跑、取消 CI，不发布、部署或更新本机 CLI。
...
本 PLAN 提议实施交付走 origin 功能分支及 PR，保留 main 的精确源 CI 与发布来源校验。
PLAN 编写不修改本地规则或平台门槛；实施批准应包含这一具体交付安排，不能用绕过门槛解决冲突。
...
首轮无包版本变化、正式制品或生产部署；如 B1 扩展为产品修复，必须单独列出受影响包和对应发行交付。
