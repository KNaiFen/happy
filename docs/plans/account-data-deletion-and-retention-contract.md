...
- 当前状态：进行中（源码实现与本地验证收口；等待 PR/CI、对应发行候选和托管运维证据）。
...
- 不可撤销边界：删除标记前已经准入的附件下载流可继续发送已打开的流；已经交给 Expo 的通知、已发往远端
  ...
  已发送字节或第三方已经接收的副作用。
- 未关闭事项：完成对应 PR 的云端 CI/发行候选验证；
  托管环境必须先确认所有旧直传 S3 签发器已排空并记录
  `ACCOUNT_DELETION_LEGACY_DIRECT_UPLOADS_DRAINED_AT`，以及保存备份、对象副本和运维日志三天保留的
  实际配置记录。它们均不能由本地源码测试替代。
...
11. [ ] 在托管环境中完成以下可审计操作：确认旧 S3 直传版本已从所有副本下线，记录
   `ACCOUNT_DELETION_LEGACY_DIRECT_UPLOADS_DRAINED_AT`；将账户相关备份和运维日志保留上限设为三天；
   保存配置版本、执行时间和一次实际删除后的对象/日志/备份证据。自托管不由本项目代为配置或验收。
...
- 托管三天备份/日志保留只有在运维配置与证据可审计时才能标记完成；自托管副本保留责任必须持续
  明确归属部署者。
