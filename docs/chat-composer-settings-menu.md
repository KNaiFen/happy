...
- 三个触发器复用消息输入面板正上方原有的连接状态行，不新增设置行。视觉顺序为
  连接状态、模型、思考程度、权限，例如 `APP控制    GPT-5.6-sol·Max  ▶ YOLO`。
- 控件保持单行，不换行，也不提供横向滚动。
- 权限和思考程度按完整当前值确定宽度；模型占用剩余宽度并可以收缩。
- 模型触发器保留名称开头和最后六个 Unicode 字符，在中间省略；菜单项和
  无障碍标签始终使用完整名称。
- 模型和思考程度只显示当前值，并以中点连接。权限恢复原状态行的语义图标、
  状态颜色和完整当前值。权限使用独立菜单；模型和思考程度使用相同的组合菜单，
  点击的触发器对应分组排在前面。
