...
- Current state: The model-aware Codex reasoning implementation is complete but unreleased. Local Android release builds disable Expo OTA updates, target arm64-v8a devices, use a quiet outlined mobile send action, and keep Agent Defaults in device-local storage while mirroring them to account sync. The verified APP 1.9.0 artifact is `dist/release-artifacts/happy-app-1.9.0-android-arm64-v8a-no-ota.apk`, and the verified CLI artifact remains `dist/release-artifacts/happy-1.3.0.tgz`; only physical-device validation remains open.
