...
async fn execute_relay_request(
  ...
  for (name, value) in headers {
    header_bytes = header_bytes.saturating_add(name.len()).saturating_add(value.len());
  ...
  let mut response = builder
    .send()
    .await
    .map_err(|_| "Relay request failed")?;
...
mod tests {
  ...
  fn requires_explicit_http_authorization() {
    assert!(
      parse_relay_policy(&policy_request("http://192.168.1.20:3005", false)).is_err()
    );
  ...
  fn rejects_origin_changes_and_credentials() {
    ...
    assert!(
      parse_relay_policy(&policy_request(
